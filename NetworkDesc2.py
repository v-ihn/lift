...
class NetworkDesc2:
    def __init__(self, learning_rate=0.001, model_file='desc2.h5'):
        self.model_dir = 'models/desc2/'
        ...
        self.matching = K.backend.variable(True, dtype='bool')

        self.output1, self.output2 = self.init_outputs()
    ...
    def init_training_loss(self, margin=4.0):
        def calculate_loss(outputs):
            dist = K.backend.sqrt(K.backend.sum(K.backend.square(outputs[0] - outputs[1]), axis=1, keepdims=True))
            return K.backend.switch(self.matching, lambda: dist, lambda: K.backend.relu(margin - dist))

        return K.layers.Lambda(calculate_loss)([self.output1, self.output2])

    def init_optimizer(self, learning_rate):
        # return K.optimizers.Adam(learning_rate)
        return K.optimizers.SGD(learning_rate, momentum=0.9)

    def init_models(self):
        training_model = K.Model(inputs=[self.p1, self.p2], outputs=self.training_loss)
    ...
    def train_model(self, input1, input2, matching):
        K.backend.set_value(self.matching, matching)
        # self.matching = matching
        return self.training_model.train_on_batch([input1, input2], np.zeros(input1.shape[0]))

    def test_model(self, input1, input2, matching):
        K.backend.set_value(self.matching, matching)
        # self.matching = matching
        return self.training_model.test_on_batch([input1, input2], np.zeros(input1.shape[0]))

    def get_losses(self, input1, input2, matching):
        K.backend.set_value(self.matching, matching)
        # self.matching = matching
        return self.training_model.predict_on_batch([input1, input2])

    def get_descriptor(self, input_patch):
        return self.desc_model.predict_on_batch(input_patch)

    def hardmine_train(self, input1, input2, matching, mining_ratio):
        if mining_ratio == 1:
            return self.train_model(input1, input2, matching)
        else:
            losses = self.get_losses(input1, input2, matching).flatten()
            ...
            return self.train_model(training_input1, training_input2, matching)
