...
class Dataset:
    def __init__(self, images, labels, size=64, rand_rot_range=None, mean=None, std=None):
        ...
        for i in range(len(images)):
            resized_images.append(cv2.resize(images[i], (size, size)))
        ...
        print(list(self.available_patches))
        ...
        #     i = 0
        #     for img in self.available_patches[label]:
    ...
    def create_fixed_epoch(self, size):
        self.fixed_epoch_desc = []
        for _ in range(size):
            self.fixed_epoch_desc.append(self.get_triplet())
    ...
    def get_batch_triplets_from_fixed_epoch(self, batch_size: int):
